...
export default async function DashboardPage() {
  ...
  const [productCount, orderCount, messageCount, recentOrders] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Pesanan Aktif", value: orderCount[0].count, icon: "🛒" },
    { label: "Pesan Masuk", value: messageCount[0].count, icon: "💬" },
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Halo! 👋 Selamat datang di {seller.storeName}</h1>
        <p className="text-gray-500 mt-1">
          Link toko:{" "}
          <span className="text-blue-600">nemu-ai.com/toko/{seller.storeSlug}</span>
